//! CLI argument parsing for IBSR collector.
//!
//! Provides command-line interface for configuring the XDP collector.
...
pub struct CollectArgs {
    /// Destination port to monitor (required).
...
impl CollectArgs {
    ...
    pub fn validate(&self) -> Result<(), CliError> {
        if self.port == 0 {
            return Err(CliError::InvalidPort(self.port as u32));
        }
        if self.max_files == 0 {
...
mod tests {
    ...
    #[test]
    fn test_collect_with_port_short() {
    ...
    #[test]
    fn test_collect_with_port_long() {
        let cli = parse_from(["ibsr", "collect", "--port", "8899"]).expect("parse");
        match cli.command {
            Command::Collect(args) => {
                assert_eq!(args.port, 8899);
            }
    ...
    #[test]
    fn test_collect_port_max_value() {
        let cli = parse_from(["ibsr", "collect", "--port", "65535"]).expect("parse");
        match cli.command {
            Command::Collect(args) => {
    ...
    #[test]
    fn test_collect_port_min_value() {
        let cli = parse_from(["ibsr", "collect", "--port", "1"]).expect("parse");
        match cli.command {
            Command::Collect(args) => {
    ...
    #[test]
    fn test_collect_port_zero_validation() {
        let cli = parse_from(["ibsr", "collect", "--port", "0"]).expect("parse");
        match cli.command {
            Command::Collect(args) => {
    ...
    #[test]
    fn test_collect_port_overflow() {
        // Port > 65535 should fail parsing
        let result = parse_from(["ibsr", "collect", "--port", "65536"]);
        assert!(result.is_err());
    }

    #[test]
    fn test_collect_port_negative() {
        let result = parse_from(["ibsr", "collect", "--port", "-1"]);
        assert!(result.is_err());
    }

    #[test]
    fn test_collect_port_non_numeric() {
        let result = parse_from(["ibsr", "collect", "--port", "abc"]);
        assert!(result.is_err());
    }
    ...
    fn test_collect_custom_out_dir() {
        let cli = parse_from(["ibsr", "collect", "--port", "8899", "--out-dir", "/tmp/snapshots"])
            .expect("parse");
        match cli.command {
            Command::Collect(args) => {
    ...
    fn test_collect_custom_max_files() {
        let cli = parse_from(["ibsr", "collect", "--port", "8899", "--max-files", "100"])
            .expect("parse");
        match cli.command {
            Command::Collect(args) => {
    ...
    fn test_collect_max_files_zero_validation() {
        let cli = parse_from(["ibsr", "collect", "--port", "8899", "--max-files", "0"])
            .expect("parse");
        match cli.command {
            Command::Collect(args) => {
                let result = args.validate();
    ...
    fn test_collect_custom_max_age() {
        let cli = parse_from(["ibsr", "collect", "--port", "8899", "--max-age", "7200"])
            .expect("parse");
        match cli.command {
            Command::Collect(args) => {
    ...
    fn test_collect_max_age_zero_validation() {
        let cli = parse_from(["ibsr", "collect", "--port", "8899", "--max-age", "0"])
            .expect("parse");
        match cli.command {
            Command::Collect(args) => {
                let result = args.validate();
    ...
    fn test_collect_custom_map_size() {
        let cli = parse_from(["ibsr", "collect", "--port", "8899", "--map-size", "50000"])
            .expect("parse");
        match cli.command {
            Command::Collect(args) => {
    ...
    fn test_collect_map_size_zero_validation() {
        let cli = parse_from(["ibsr", "collect", "--port", "8899", "--map-size", "0"])
            .expect("parse");
        match cli.command {
            Command::Collect(args) => {
                let result = args.validate();
    ...
    fn test_collect_custom_iface() {
        let cli = parse_from(["ibsr", "collect", "--port", "8899", "--iface", "eth0"])
            .expect("parse");
        match cli.command {
            Command::Collect(args) => {
...
eth0 00000000 0102A8C0";
    ...
    fn test_unknown_flag() {
        let result = parse_from(["ibsr", "collect", "--port", "8899", "--unknown"]);
        assert!(result.is_err());
    }
    ...
    fn test_cli_equality() {
        let cli1 = parse_from(["ibsr", "collect", "--port", "8899"]).expect("parse");
        let cli2 = parse_from(["ibsr", "collect", "--port", "8899"]).expect("parse");
        assert_eq!(cli1, cli2);
    }
